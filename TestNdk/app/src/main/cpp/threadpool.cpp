...
void *adjust_thread(void *threadpool)
{
    ...
    while (!pool->shutdown)
    {
        LOGI("adjust_thread 11111");
        sleep(DEFAULT_TIME);                                    /*延时10秒*/
        LOGI("adjust_thread 22222");
        pthread_mutex_lock(&(pool->lock));
      ...
      //  LOGI("adjust_thread 3333error");
        ...
        /*销毁多余的空闲线程*/
        LOGI("adjust_thread 3333error:%d,%d,%d",busy_thr_num,live_thr_num,pool->min_thr_num);
...
int threadpool_destroy(threadpool_t *pool)
{
    ...
    pool->shutdown = true;
    /*先销毁管理线程*/
    LOGI("before adjust_tid destory:");
    pthread_join(pool->adjust_tid, NULL);
    LOGI("after adjust_tid destory:");
